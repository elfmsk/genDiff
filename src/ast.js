...
const processForObjects = [
  {
    ...
    process: (key, childrenB, childrenA, f) => ({
      name: key, type: 'node', children: f(childrenB[key], childrenA[key]),
    }),
  },
  {
    check: (key, objB, objA) => objB[key] === objA[key],
    process: (key, objA) => ({
      name: key, type: 'unaltered', value: objA[key],
    }),
  },
  {
    check: (key, objB, objA) => !_.has(objB, key) && _.has(objA, key),
    process: (key, objB, objA) => ({
      name: key, type: 'added', value: objA[key],
    }),
  },
  {
    check: (key, objB, objA) => _.has(objB, key) && !_.has(objA, key),
    process: (key, objB) => ({
      name: key, type: 'removed', value: objB[key],
    }),
  },
  {
    check: (key, objB, objA) => objB[key] !== objA[key],
